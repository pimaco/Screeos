...
var roleNukeFiller = require('role.nukeFiller');
var rolePowerHelper = require('role.powerHelper');
...
module.exports.loop = function () {
      ...
      for (let roomKey in rooms) {
        ...
        if (isMyRoom) {
          ...
          if (room.storage) {
            ...
            storedTotal = room.storage.store.getCapacity(RESOURCE_ENERGY);
          } else {
            stored = 0
            storedTotal = 0
          }
    ...
    if((Game.getObjectById('5b1a9a14d025753871a5f213').energy == Game.getObjectById('5b1a9a14d025753871a5f213').energyCapacity && Game.getObjectById('5b1a9a14d025753871a5f213').ghodium == Game.getObjectById('5b1a9a14d025753871a5f213').ghodiumCapacity))
    {
      //  Game.notify('About to send nuke!!');
        //Game.getObjectById('5b1a9a14d025753871a5f213').launchNuke(new RoomPosition(28,31, 'W52N25'));
    } 
    if((Game.getObjectById('5d057877600d0416f43beda3').energy >= 50 && Game.getObjectById('5d057877600d0416f43beda3').power >= 1))
    { 
       // console.log('processing power');
        Game.getObjectById('5d057877600d0416f43beda3').processPower();
    }

     //console.log(controlledRooms.length);
	if(Game.time % 100 == 0|| !Memory.myRooms || Memory.myRooms.length < controlledRooms.length)
    {

        var rname = "";
        var dist = null;
        var myrooms = [];
        var resInventory = null;
        var globalInventory = [];
        Memory.inventory = [];
        for (var i = 0, len = controlledRooms.length; i < len; i++)
        {
            Memory.myRooms[i] = controlledRooms[i];
            if(controlledRooms[i].terminal)
            {
                resInventory = [];
                _.forEach(controlledRooms[i].terminal.store, (s, s1)=> {resInventory += [s1 + ": "+ s + "  " ]})
                globalInventory = [controlledRooms[i].name, resInventory];
                Memory.inventory
                Memory.inventory[i] =globalInventory;
            }
        ...
        var ordersProps = [];
        var i = -1;
        _.forEach(Game.market.orders, (s) => { i += 1, ordersProps[i] = s }); 
        
        for ( var u=0, leng = ordersProps.length; u < leng; u++)
        {
            //if order is 30 (-3 hours) old, renew before it expires (30 days = 2592000000 ms )
            if(Date.now() - ordersProps[u].createdTimestamp > 2581200000 )
            {
                
                console.log("create order "+ ordersProps[u].roomName);
                avgPrice = (Game.market.getHistory(ordersProps[u].resourceType)[13].avgPrice - Game.market.getHistory(ordersProps[u].resourceType)[13].stddevPrice).toFixed(3);
                
                 Game.market.createOrder({
                    type: ordersProps[u].type,
                    resourceType: ordersProps[u].resourceType,
                    price: avgPrice,
                    totalAmount: 400000,
                    roomName: ordersProps[u].roomName
                  });
            }
        }
    ...
    for (var j = 0, len = controlledRooms.length; j < len; j++)
    {
        ...
        nukeFillers[j] = _.filter(Game.creeps, function(creep) { return creep.memory.role == 'nukefiller' && creep.memory.home.name == controlledRooms[j].name;});
        powerHelpers[j] = _.filter(Game.creeps, function(creep) { return creep.memory.role == 'powerhelper' && creep.memory.home.name == controlledRooms[j].name;});        
        ...
        if(controlledRooms[j].storage && controlledRooms[j].terminal  && controlledRooms[j].storage.store[RESOURCE_ENERGY] < 12000 && _.sum(controlledRooms[j].storage.store) < 250000 && controlledRooms[j].terminal.store[RESOURCE_ENERGY] < 9000 && _.sum(controlledRooms[j].terminal.store) < 290000 ) //&& controlledRooms[j].controller.level < 8
        {
            termTarget[j] = true;
        }
        else
    ...
    global.attackers = _.filter(Game.creeps, function(creep) { return creep.memory.role == 'attacker';});
    ...
    for(var name in Game.creeps)
    {
        ...
        else if(creep.memory.role == 'nukefiller')
        {
            roleNukeFiller.run(creep);
        }
        else if(creep.memory.role == 'powerhelper')
        {
            rolePowerHelper.run(creep);
        }
...
function funcCreepSpawner(activeRoom, index,nbContainersInRoom,controlledRooms)
{
    ...
    for(var z = 0, leng = sources.length; z < leng; z++)
    {
        ...
        if(activeRoom.controller.level >= 4 && (nbContainersAtSource[z].length < 1 || !nbContainersAtSource[z]) && (!nbContainerConstructSite[z] || nbContainerConstructSite[z].length < 1 ))
        {
    ...
    if(activeSpawns.length > 0)
    {
        for(var s = 0, Slen = activeSpawns.length; s < Slen; s++)
        {
            ...
            else if (activeSpawns[s])//.spawning == null)
            {
                ...
                else if((spawnhelpers[index].length < 1 || (spawnhelpers[index][0].ticksToLive < 75 && spawnhelpers[index].length < 2)) && (activeRoom.storage || nbContainersInRoom[index].length > 0))
                {
                ...
                else if(nukeFillers[index] && activeRoom.name == 'W62N27' && (nukeFillers[index].length < 0 || (nukeFillers[index][0] && nukeFillers[index][0].ticksToLive < 150 && nukeFillers[index].length < 0)) && activeRoom.terminal && Game.getObjectById('5b1a9a14d025753871a5f213').cooldown == 0 )
                {
                    ...
                    {
                        activeSpawns[s].spawnCreep([MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY], 'NukeFiller' + activeRoom.name + '_' + (Math.floor(Math.random() * 100) + 1), { memory: {role: 'nukefiller', home: activeRoom}});
                        console.log('Spawning new nukeFiller ' + activeRoom.name );
                ...
                else if(powerHelpers[index] && activeRoom.name == 'W65N28' && powerHelpers[index].length < 1 )
                {
                    ...
                    {
                        activeSpawns[s].spawnCreep([MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,CARRY,CARRY], 'PowerHelper' + activeRoom.name + '_' + (Math.floor(Math.random() * 100) + 1), { memory: {role: 'powerhelper', home: activeRoom}});
                        console.log('Spawning new powerHelper ' + activeRoom.name );
                        powerHelpers[index] = _.filter(Game.creeps, function(creep) { return creep.memory.role == 'powerhelper' && creep.memory.home.name == activeRoom.name;});
                    }
                }
                else if(upgraders[index].length < 1  && (nbContainersInRoom[index].length > 0 || activeRoom.storage)) //&& activeRoom.controller.level < 8)
                {
                ...
                else if((upgraders[index].length < 1 || (upgraders[index][0].ticksToLive < 200 && upgraders[index].length < 2)) && activeRoom.controller.level == 8) 
                {
                ...
                else if (index > 12 && (upcrossers[17].length < 6))//) ||!upcrossers[0])
                {
                    var roomNum = 17;
                    ...
                    {
                        activeSpawns[s].spawnCreep([[MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,WORK,WORK,WORK,CARRY,CARRY,CARRY]], 'Upcrosser_' + activeRoom.name + '_' + (Math.floor(Math.random() * 100) + 1), { memory: {role: 'upcrosser', home: Memory.myRooms[roomNum]}});
                        console.log('Spawning new upcrosser for dest  '+ Memory.myRooms[roomNum].name +'  from ' + activeRoom)
                    ...
                    {
                        activeSpawns[s].spawnCreep([MOVE,MOVE,MOVE,MOVE,WORK,WORK,CARRY,CARRY], 'Upcrosser_' + activeRoom.name + '_' + (Math.floor(Math.random() * 100) + 1), { memory: {role: 'upcrosser', home: Memory.myRooms[roomNum]}});
                        console.log('Spawning new upcrosser for dest  '+ Memory.myRooms[roomNum].name +'  from ' + activeRoom)
                        upcrossers[roomNum] = _.filter(Game.creeps, function(creep) { return creep.memory.role == 'upcrosser' && creep.memory.home.name == Memory.myRooms[roomNum].name;});
                    }
                    else if(activeSpawns[s].spawnCreep([WORK,CARRY,MOVE],'testSpawn', { dryRun: true}) == OK)
                    {
                        activeSpawns[s].spawnCreep([WORK,CARRY,MOVE], 'Upcrosser_' + activeRoom.name + '_' + (Math.floor(Math.random() * 100) + 1), { memory: {role: 'upcrosser', home: Memory.myRooms[roomNum]}});
                        console.log('Spawning new upcrosser for dest  '+ Memory.myRooms[roomNum].name +'  from ' + activeRoom )
                        upcrossers[roomNum] = _.filter(Game.creeps, function(creep) { return creep.memory.role == 'upcrosser' && creep.memory.home.name == Memory.myRooms[roomNum].name;});
                    }
                }
                else if (index < 11 && (upcrossers[2].length < 0)) //||!upcrossers[2])
                {
                    ...
                    {
                        activeSpawns[s].spawnCreep([MOVE,MOVE,MOVE,MOVE,WORK,WORK,CARRY,CARRY], 'Upcrosser_' + activeRoom.name + '_' + (Math.floor(Math.random() * 100) + 1), { memory: {role: 'upcrosser', home: Memory.myRooms[roomNum2]}});
                        console.log('Spawning new upcrosser for dest  '+ Memory.myRooms[roomNum2].name +'  from ' + activeRoom)
                        upcrossers[roomNum2] = _.filter(Game.creeps, function(creep) { return creep.memory.role == 'upcrosser' && creep.memory.home.name == Memory.myRooms[roomNum2].name;});
                    }
                    else if(activeSpawns[s].spawnCreep([WORK,CARRY,MOVE],'testSpawn', { dryRun: true}) == OK)
                ...
                else if(!builders[17] || (builders[17].length < 2 && constructsite[17] && constructsite[17].length > 0) && index > 12)
                {
                    var roomNum = 17;
                    if(activeSpawns[s].spawnCreep([MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,WORK,WORK,WORK,WORK,CARRY,CARRY],'testSpawn', { dryRun: true}) == OK)
                    {
                        ...
                        console.log('Spawning new builder for dest '+ Memory.myRooms[roomNum].name + ' from '+ activeRoom.name);
                    ...
                    {
                        activeSpawns[s].spawnCreep([WORK,CARRY, MOVE], 'Builder_' + activeRoom.name + '_' + (Math.floor(Math.random() * 100) + 1), { memory: {role: 'builder', home: Memory.myRooms[roomNum]}});
                        console.log('Spawning new builder for dest '+ Memory.myRooms[roomNum].name + ' from '+ activeRoom.name);
                        builders[roomNum]= _.filter(Game.creeps, function(creep) { return creep.memory.role == 'builder' && creep.memory.home.name == Memory.myRooms[roomNum].name;});
                    }	
                } 
                /*else if(!builders[2] || (builders[2].length < 2 && constructsite[2] && constructsite[2].length > 0) && index > 0)
                {
                    ...
                    if(activeSpawns[s].spawnCreep([MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,WORK,WORK,WORK,WORK,CARRY,CARRY],'testSpawn', { dryRun: true}) == OK)
                    {
                        activeSpawns[s].spawnCreep([MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,WORK,WORK,CARRY,CARRY], 'Builder_' + activeRoom.name + '_' + (Math.floor(Math.random() * 100) + 1), { memory: {role: 'builder', home: Memory.myRooms[roomNum2]}});
                        console.log('Spawning new builder for dest '+ Memory.myRooms[roomNum2].name + ' from '+ activeRoom.name);
                    ...
                    {
                        activeSpawns[s].spawnCreep([WORK,CARRY, MOVE], 'Builder_' + activeRoom.name + '_' + (Math.floor(Math.random() * 100) + 1), { memory: {role: 'builder', home: Memory.myRooms[roomNum2]}});
                        console.log('Spawning new builder for dest '+ Memory.myRooms[roomNum2].name + ' from '+ activeRoom.name);
                        builders[roomNum2]= _.filter(Game.creeps, function(creep) { return creep.memory.role == 'builder' && creep.memory.home.name == Memory.myRooms[roomNum2].name;});
                    }	
                } 
  ...
  var terminal = activeRoom.terminal;
    ...
    if(activeRoom.name != 'W62N27' && terminal && !terminal.cooldown && terminal.store &&  ScienceEnabled != 'NotEnabled' && (_.sum(terminal.store) - terminal.store[RESOURCE_ENERGY]) >= 100 &&  terminal.store[RESOURCE_ENERGY] > 100)
    {
        ...
        {
            if(activeRoom.name != ScienceEnabled && !terminal.cooldown && terminal.store[RESOURCE_ENERGY] && terminal.store[RESOURCE_ENERGY] >= 800 && terminal.store[neededRsc[z]] && terminal.store[neededRsc[z]] >= 800 && (Game.rooms[ScienceEnabled].terminal.store[neededRsc[z]] < 3000 || !Game.rooms[ScienceEnabled].terminal.store[neededRsc[z]]))
            {
            ...
            else if(Game.market.credits > 900000 && !Game.rooms[ScienceEnabled].terminal.cooldown && (!Game.rooms[ScienceEnabled].terminal.store[neededRsc[z]] || Game.rooms[ScienceEnabled].terminal.store[neededRsc[z]] < 800) && Game.rooms[ScienceEnabled].terminal.store[RESOURCE_ENERGY] > 500)
            {
                ...
                if(orders.length > 0)
                {
                    console.log("Buying resource " + neededRsc[z] + " from order " + orders[orders.length-1].id + " at price " + orders[orders.length-1].price + " in room "+ Game.rooms[ScienceEnabled].name);
                    Game.market.deal(orders[orders.length-1].id,100,Game.rooms[ScienceEnabled].name);
                    //console.log(Game.market.deal(orders[0].id,100,Game.rooms[ScienceEnabled].name));
                    
                }
            }
        }
    }
    else if( terminal && !terminal.cooldown && activeRoom.name == ScienceEnabled && terminal.store[RESOURCE_ENERGY] >= 500 &&  terminal.store[RESOURCE_GHODIUM_HYDRIDE] >= 500)
    {

        if(Game.rooms['W66N31'].terminal && ((Game.rooms['W66N31'].terminal.store[RESOURCE_GHODIUM_HYDRIDE] <= 1500 || Game.rooms['W66N31'].terminal.store[RESOURCE_GHODIUM_HYDRIDE] == undefined) && (Game.rooms['W66N31'].storage.store[RESOURCE_GHODIUM_HYDRIDE]  <=1500 || Game.rooms['W66N31'].storage.store[RESOURCE_GHODIUM_HYDRIDE] == undefined)))
        {
            console.log("Send GH to W66N31");
            terminal.send(RESOURCE_GHODIUM_HYDRIDE, 500, 'W66N31');
        }
        else if(Game.rooms['W63N28'].terminal && ((Game.rooms['W63N28'].terminal.store[RESOURCE_GHODIUM_HYDRIDE] <= 1500 || Game.rooms['W63N28'].terminal.store[RESOURCE_GHODIUM_HYDRIDE] == undefined) && (Game.rooms['W63N28'].storage.store[RESOURCE_GHODIUM_HYDRIDE]  <=1500 || Game.rooms['W63N28'].storage.store[RESOURCE_GHODIUM_HYDRIDE] == undefined) ) )
        {
            console.log("Send GH to W63N28");
            terminal.send(RESOURCE_GHODIUM_HYDRIDE, 500, 'W63N28');
        }
        else if(Game.rooms['W65N28'].terminal && ((Game.rooms['W65N28'].terminal.store[RESOURCE_GHODIUM_HYDRIDE] <= 1500 || Game.rooms['W65N28'].terminal.store[RESOURCE_GHODIUM_HYDRIDE] == undefined) && (Game.rooms['W65N28'].storage.store[RESOURCE_GHODIUM_HYDRIDE]  <=1500 || Game.rooms['W65N28'].storage.store[RESOURCE_GHODIUM_HYDRIDE] == undefined)) )
        {
...
function funcPowerCreeps(roomName)
{
    for (var p in Game.powerCreeps) {
        ...
        if (!powerCreep.ticksToLive) {
            // Not spawned in world; check if we can spawn creep
